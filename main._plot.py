...
with tab1:
    ...
    try:
        ...
        if missing_nordic:
            for code in missing_nordic:
                new_row = pd.DataFrame({
                    ...
                    "Total": [0],
                    "iso_alpha3": [pycountry.countries.get(alpha_2=code).alpha_3 if pycountry.countries.get(alpha_2=code) else None]
                })
        ...
        if not df_nordic.empty:
            ...
            fig_nordic_bar.update_layout(yaxis={"categoryorder": "total ascending"}, height=400)
